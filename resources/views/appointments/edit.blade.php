                    ...
                    <form action="{{ route('appointments.update', $appointment) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="procedure_id" id="procedure_id" class="dental-input" required>
                                    ...
                                    @foreach($procedures as $procedure)
                                        <option value="{{ $procedure->id }}" {{ old('procedure_id', $appointment->procedure_id) == $procedure->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="status" id="status" class="dental-input" required>
                                    @foreach($statuses as $key => $value)
                                        <option value="{{ $key }}" {{ old('status', $appointment->status) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="date" name="appointment_date" id="appointment_date" value="{{ old('appointment_date', $appointment->appointment_date->format('Y-m-d')) }}" class="dental-input" required>
                            ...
                            <div>
                                ...
                                <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $startTime) }}" class="dental-input" required>
                                @error('start_time')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="notes" id="notes" rows="3" class="dental-input">{{ old('notes', $appointment->notes) }}</textarea>
                            @error('notes')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-6 flex items-center justify-end">
                            ...
                            <button type="submit" class="dental-button">Update Appointment</button>
                        </div>
                    </form>
                </div>
            </div>
